                ...
                <div class="col-lg-6">
                    <div class="single_details_col">
                        ...
                        <div class="price mt-4 d-flex align-items-center ">
                            <h3>{{$offer->piece_cost}} {{optional(getCurrentCountry()->currency)->name}}</h3>
                            <h3 class="ml-4">
                                <del>{{optional($product->price)->piece_cost}} {{optional(getCurrentCountry()->currency)->name}}</del>
                            </h3>
                        </div>
                        <div class="Add">
    ...
    <section class="sliderProducts singlePageSlider pt-5">
        <div class="container">
            ...
            <div class="productsSlide owl-carousel owl_5 mt-4">
                @foreach($related_offers as $index=>$item)
                    <a href="{{route('offer',$item->id)}}">
                        <div class="singleProduct">
                            <div class="productImg">
                                <img src="{{optional(optional($item)->price)->product->image}}" alt="">
                            </div>
                            <div class="productOver">
                                <div class="prev d-block">
                                    <i class="fas fa-star mr-1"></i>
                                    <span>{{optional(optional($item)->price)->product->rate}}</span>
                                </div>
                                <div class="productDetails">
                                    <div class="Pname">
                                        <p>{{optional(optional($item)->price)->product->name}}</p>
                                    </div>
                                    <div class="Pprice">
                                        <p>{{optional($item)->piece_cost}} {{optional(getCurrentCountry()->currency)->name}}</p>
                                    </div>
